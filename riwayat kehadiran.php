...
// Ambil riwayat kehadiran dari database
$id_detail = $karyawan['id_detail'];
$query = "SELECT a.*, da.nama 
...
<head>
  ...
  <title>Riwayat Kehadiran | Gruduk Cafe</title>
</head>
...
<body class="relative min-h-screen bg-cover bg-center" style="background-image:url('assets/Background 1.jpg');">
  ...
  <div class="flex min-h-screen">
    ...
    <!-- Sidebar -->
    <aside
      class="w-56 min-h-screen bg-gradient-to-b from-[#4e3b2e]/90 to-[#2f1e15]/90 text-white flex flex-col justify-between p-6">
      <div class="space-y-4">
                ...
                <?php if ($result && mysqli_num_rows($result) > 0): ?>
                  <?php while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr class="border-t border-gray-200 hover:bg-gray-50">
                      <td class="py-2 px-4">
                        <?= date('d/m/Y H:i', strtotime($row['tanggal_absen'])) ?>
                      </td>
                      ...
                      <td class="py-2 px-4 text-center">
                        <?php if (!empty($row['foto_absen'])): ?>
                          <a href="uploads/absensi/<?= htmlspecialchars($row['foto_absen']) ?>" target="_blank"
                            class="bg-gray-700 text-white px-6 py-1 rounded hover:bg-gray-800 transition">
                            Bukti
                          </a>
                        <?php else: ?>
